...
type CleanupPolicyLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.CleanupPolicy, err error)
...
// List lists all CleanupPolicies in the indexer.
func (s *cleanupPolicyLister) List(selector labels.Selector) (ret []*v1alpha2.CleanupPolicy, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.CleanupPolicy))
	})
	return ret, err
}
...
type CleanupPolicyNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha2.CleanupPolicy, error)
...
// List lists all CleanupPolicies in the indexer for a given namespace.
func (s cleanupPolicyNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.CleanupPolicy, err error) {
...
// Get retrieves the CleanupPolicy from the indexer for a given namespace and name.
func (s cleanupPolicyNamespaceLister) Get(name string) (*v1alpha2.CleanupPolicy, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("cleanuppolicy"), name)
	}
	return obj.(*v1alpha2.CleanupPolicy), nil
}
