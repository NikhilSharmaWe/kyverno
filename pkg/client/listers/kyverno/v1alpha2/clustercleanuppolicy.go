...
type ClusterCleanupPolicyLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.ClusterCleanupPolicy, err error)
	// Get retrieves the ClusterCleanupPolicy from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha2.ClusterCleanupPolicy, error)
...
// List lists all ClusterCleanupPolicies in the indexer.
func (s *clusterCleanupPolicyLister) List(selector labels.Selector) (ret []*v1alpha2.ClusterCleanupPolicy, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.ClusterCleanupPolicy))
	})
	return ret, err
}

// Get retrieves the ClusterCleanupPolicy from the index for a given name.
func (s *clusterCleanupPolicyLister) Get(name string) (*v1alpha2.ClusterCleanupPolicy, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("clustercleanuppolicy"), name)
	}
	return obj.(*v1alpha2.ClusterCleanupPolicy), nil
}
